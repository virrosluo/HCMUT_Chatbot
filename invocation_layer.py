...
class HFInferenceEndpointInvocationLayer(PromptModelInvocationLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.prompt_preprocessors["oasst"] = (
            lambda prompt: f"<|prompter|>{prompt}<|endoftext|><|assistant|>"
        )
    ...
    def url(self) -> str:
        if HFInferenceEndpointInvocationLayer.is_inference_endpoint(
            ...
            url = (
                f"https://api-inference.huggingface.co/models/{self.model_name_or_path}"
            )
        return url
    ...
    def invoke(self, *args, **kwargs):
        ...
        params = {
            ...
            "max_new_tokens": kwargs_with_defaults.get(
                "max_new_tokens", self.max_length
            ),
            "max_time": kwargs_with_defaults.get("max_time", None),
            "num_return_sequences": kwargs_with_defaults.get(
        ...
        response: requests.Response = self._post(
            data={"inputs": prompt, "parameters": params, "stream": stream},
            stream=stream,
        )
        if stream:
            ...
            generated_texts = self._process_streaming_response(
                response, handler, stop_words
            )
        else:
            output = json.loads(response.text)
            generated_texts = [
                o["generated_text"] for o in output if "generated_text" in o
            ]
    ...
    def _extract_token(self, event_data: Dict[str, Any]) -> Optional[str]:
        ...
        return (
            event_data["token"]["text"] if not event_data["token"]["special"] else None
        )
    ...
    def is_inference_endpoint(model_name_or_path: str) -> bool:
        return model_name_or_path is not None and all(
            token in model_name_or_path for token in ["http"]
        )
    ...
    def supports(cls, model_name_or_path: str, **kwargs) -> bool:
        if cls.is_inference_endpoint(model_name_or_path):
            ...
            return is_inference_api and task_name in [
